...
function getFinals(array) {
    const finalists = array.filter(function(item){
...
function getYears(data, getFinalsCallback) {
    return getFinals(data).map((item) => 
        item.Year);
}
...
function getWinners(array, getFinalsCallback) {
    return getFinalsCallback(array).map(function(item){
...
function getWinnersByYear(array, getFinalsCallback, getYearsCallback, getWinnersCallback) {
    const winningCountry = getWinnersCallback(array, getFinalsCallback);
    const winningYear = getYearsCallback(array, getFinalsCallback);
    return winningCountry.map((item, index) => `In ${winningYear[index]}, ${winningCountry[index]} won the world cup!`);
}
...
function getAverageGoals(getFinalsCallback) {
    const awayFinalsGoals = getFinalsCallback.reduce((total, item) => {
        return total + item['Away Team Goals'] + item['Home Team Goals']
    }, 0)
    return(awayFinalsGoals / getFinalsCallback.length).toFixed(2);
 }
